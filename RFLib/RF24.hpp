...
private:
    ...
    void pinModeRF24_ce_csn(void);
    TIMER_RF24 timerRF24;
    SPI_RF24 spiRF24;
    UART_RF24 uartRF24;
protected:
    /**
...
public:
     ...
     */
    //RF24(uint16_t _cepin, uint16_t _cspin, TIM_HandleTypeDef *htim_us, uint32_t _spispeed = RF24_SPI_SPEED);
    RF24(GPIO_TypeDef *_ce_port, uint16_t _cepin, GPIO_TypeDef *_csn_port, uint16_t _cspin, uint32_t _spi_speed = RF24_SPI_SPEED);
    #if defined (RF24_LINUX)
    virtual ~RF24() {};
    #endif
    ~RF24() ;

    uint8_t print(char* formato, char* testo);
    /**
...
private:
    ...
    void delayMicroseconds(uint16_t csDelay);

    uint32_t millis(void);

    //void printf_P(const char* param,char* buff);
